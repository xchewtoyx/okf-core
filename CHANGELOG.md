...
- **ADR mechanism established; delivery-loop instructions aligned with the v0.5.0 re-plan**: added `docs/decisions/` (ADR-0001 superseding the #148 round-trip refactor descope, ADR-0002 recording the canonical-serialization-form framework and a firm `ruamel.yaml` choice for YAML, and an append-only `failure-ledger.md` for tracking recurring review findings across issues). This matters because the delivery loop's own reviewer/approver instructions and `AGENTS.md` previously enforced the byte-identical-preservation contract the re-plan's requirements draft found was never actually required by the OKF spec (`docs/proposals/v0.5.0-replan/02-editing-requirements.md`, non-requirement N1) — without this alignment, the loop would generate `request_changes` spirals against the redesign it is meant to deliver. `milestone-planner.md` now reads the requirements doc and scans `docs/decisions/` before planning any issue, and gains a design-it-twice requirement for public-contract changes; `milestone-reviewer.md` gains an ADR-divergence check, a generalized (not `patching.py`-specific) plan/apply-envelope concurrency check, and appends to the failure ledger on `request_changes`; `milestone-approver.md`'s worked example is now semantic (parse-and-compare) rather than byte-identity-based. `AGENTS.md` gains a "Decision Records" section, a characterization-first rule for parsing/serialization deletions, a corrected `noqa: C901` claim, and a generalized round-trip-test rule that no longer names specific functions. The four existing spike docs under `docs/spikes/` now carry pointer banners into `docs/decisions/` for their current adoption status; they are left in place, not moved. (#193)
