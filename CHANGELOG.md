...
- **Cloudsmith publishing**: the `Publish` workflow gains a `cloudsmith` job that pushes the built sdist and wheel to a Cloudsmith Python repository on each (non-prerelease) release, alongside the existing GitHub release assets and Pages index. Requires the `CLOUDSMITH_API_KEY` repository secret and `CLOUDSMITH_REPOSITORY` repository variable (`owner/repo` slug) to be configured.
