...
generated indexes contain no frontmatter. Versions must use `<major>.<minor>`
form, and this release only accepts versions up to `0.1`.
...
`okf_version` is emitted as frontmatter. If an existing bundle-root `index.md`
declares an OKF version newer than `0.1`, the command leaves it untouched.
