...
To keep the rewriter minimal and prevent unsafe or ambiguous edits, the API does not support reference-style links, link-like shapes embedded inside code blocks/spans, or nested brackets inside link labels. The planner performs a strict count cross-check: a mismatch between the number of parsed inline AST links and raw unescaped literal link shapes (which includes shapes inside code blocks or unsupported formatting) will raise `DocumentChangePlanningError`, rejecting the document for automated rewriting. Applying the plan uses `apply_document_change()` and retains the same stale-content protection.
